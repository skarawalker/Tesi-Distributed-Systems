...
public class StreamApp{
    ...
    private static void wordCountStream() throws InterruptedException{
        ...
        //Map-Reduce
        KTable<String, Integer> wordCounts = lines.flatMapValues(value -> Arrays.asList(value.split("\\W+"))).map((key, word) -> KeyValue.pair(word,word))
                .groupByKey(Grouped.with(Serdes.String(), Serdes.String()))
                .count(store).mapValues(value -> value.intValue());
        //Print result
        wordCounts.toStream().foreach((word, count) -> System.out.println("Word: " + word + " -> " + count));

        //Send result to out topic
        wordCounts.toStream().to(outputStream, Produced.with(Serdes.String(), Serdes.Integer()));
